...
#    Automatic generate a sepolicy.te file from dmesg/logcat avc output
#    Adapted from:
#    newrules.pl, Copyright (C) 2001 Justin R. Smith (jsmith@mcs.drexel.edu)
#
#    usage: log the denials 
#    dmesg | grep avc
#    logcat | grep avc
#    past the output to a in.txt file and do the bellow:
#    '/this_file_path/sepolicy_generator.pl' -i '/in_file_you_past_avc_out_put_path/in.txt' -o '/out_file_path/out_sepolicy.te'
#
...
sub printUsage {
	print "sepolicy [-d] [-v] [-l] [-i <inputfile> ] [-o <outputfile>]
